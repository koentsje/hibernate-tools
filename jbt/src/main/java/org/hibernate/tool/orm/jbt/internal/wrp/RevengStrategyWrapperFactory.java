...
public class RevengStrategyWrapperFactory {
	...
	public static RevengStrategyWrapper createRevengStrategyWrapper(Object... objects) {
		...
		if (objects.length == 0) {
			...
			wrappedRevengStrategy = createDelegatingStrategy(
					(String) objects[0],
					(RevengStrategy) ((Wrapper) objects[1]).getWrappedObject());
		} else {
	...
	public static class RevengStrategyWrapperImpl implements RevengStrategyWrapper {

		private RevengStrategy revengStrategy = null;

		RevengStrategyWrapperImpl(RevengStrategy revengStrategy) {
