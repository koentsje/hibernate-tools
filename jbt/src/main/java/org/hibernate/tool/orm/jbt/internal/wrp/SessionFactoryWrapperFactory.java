...
public class SessionFactoryWrapperFactory {
	...
	public static class SessionFactoryWrapperImpl implements SessionFactoryWrapper {

		private SessionFactory sessionFactory = null;

		SessionFactoryWrapperImpl(SessionFactory sessionFactory) {
		...
		public Map<String, ClassMetadataWrapper> getAllClassMetadata() {
			Map<String, ClassMetadataWrapper> result = new HashMap<>();
			MappingMetamodelImpl mappingMetaModel =
					(MappingMetamodelImpl) ((SessionFactoryImplementor) sessionFactory).getMappingMetamodel();
			for (String key : mappingMetaModel.getAllEntityNames()) {
				result.put(key, ClassMetadataWrapperFactory.createClassMetadataWrapper(
						mappingMetaModel.findEntityDescriptor(key)));
			}
			return result;
		}
		...
		public Map<String, CollectionMetadataWrapper> getAllCollectionMetadata() {
			Map<String, CollectionMetadataWrapper> result = new HashMap<>();
			MappingMetamodelImpl mappingMetaModel =
					(MappingMetamodelImpl) ((SessionFactoryImplementor) sessionFactory).getMappingMetamodel();
			for (String key : mappingMetaModel.getAllCollectionRoles()) {
				result.put(key, CollectionMetadataWrapperFactory.createCollectionMetadataWrapper(
						mappingMetaModel.findCollectionDescriptor(key)));
			}
			return result;
		}
